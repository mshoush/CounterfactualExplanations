...
class CounterfactualTrainer(BaseTrainer):
    ...
    def training_epoch(self, loader: torch.utils.data.DataLoader) -> None:
        print(f"Total number of epoch: {self.opt.n_epochs}")
        self.model.train()
    ...
    def evaluate_counterfactual(self, loader, phase='val', tau=0.8, skip_fid=False, postprocess_morph=False):
        ...
        cf_dir = self.cf_vis_dir_train if phase == 'train' else self.cf_vis_dir_val
        real_img_dir = cf_dir / 'real_images'
        gen_img_dir = cf_dir / 'generated_images'
        diff_img_dir = cf_dir / 'diff_images'
        ...
        for i, batch in tqdm(enumerate(loader), desc='Validating counterfactuals', leave=False, total=len(loader)):
            ...
            try:
                ...
                real_imgs = ((real_imgs + 1) / 2).clamp(0, 1).to(torch.uint8)
                gen_cf_c = ((gen_cf_c + 1) / 2).clamp(0, 1).to(torch.uint8)
                gen_cf_fx = ((gen_cf_fx + 1) / 2).clamp(0, 1).to(torch.uint8)
            ...
            base_filename = f'epoch_{self.current_epoch}_counterfactual_{i}_label_{labels[0]}_true_{real_f_x_desired_discrete[0][0]}_pred_{gen_f_x_discrete[0][0]}'
            real_img_path = real_img_dir / f'{base_filename}.jpg'
            gen_img_path = gen_img_dir / f'{base_filename}.jpg'
            diff_img_path = diff_img_dir / f'{base_filename}.jpg'
            
            try:
                save_image(real_imgs, real_img_path, nrow=3, normalize=False)
                save_image(gen_cf_c, gen_img_path, nrow=3, normalize=False)
                save_image(diff, diff_img_path, nrow=3, normalize=False)
            except:
                real_imgs_float = real_imgs.float() / 255.0
                gen_cf_c_float = gen_cf_c.float() / 255.0
                diff_float = diff.float() / 255.0

                save_image(real_imgs_float, real_img_path, nrow=3, normalize=False)
                save_image(gen_cf_c_float, gen_img_path, nrow=3, normalize=False)
                save_image(diff_float, diff_img_path, nrow=3, normalize=False)
...
#             try:
#                 save_image(vis, vis_path, nrow=3, normalize=False)
#             except:
#                 # Convert vis to a float tensor and scale it from [0, 255] to [0, 1]
#                 vis_float = vis.float() / 255.0

#                 # Save the image
#                 save_image(vis_float, vis_path, nrow=3, normalize=False)
            ...
            if not skip_fid:
                ...
                try:
                    ...
                    # Convert gen_cf_c to float32
                    gen_cf_c = gen_cf_c.float()
                    gen_cf_c = nn.functional.interpolate(gen_cf_c, size=(299, 299), mode='bilinear', align_corners=False)
